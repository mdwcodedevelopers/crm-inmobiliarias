<template>
  <v-container>
    <v-card color="blue darken-4">
      ...
      <v-data-table :headers="headers"  no-results-text="No hay resultados" no-data-text="No hay servicioes " :items="services" item-key="propeties-user" class="elevation-1" :search="search">
        <template v-slot:top>
          <v-toolbar flat>
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-form ref="form" v-model="valid">
                  <v-layout wrap>
                    <v-flex xs12>
                      <v-text-field label="Nombre del Servicio" :rules="inputRules" v-model="service.name" required></v-text-field>
                    </v-flex>
                    <v-flex xs12>
                        {{error}}
                    </v-flex>
    ...
    <template>
      <v-layout row justify-center>
        <v-dialog v-model="dialogedit" persistent max-width="600px">
          <v-card class="my-4">
            <v-card-title>
              <span class="headline">Editar servicio</span>
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-form ref="form" v-model="valid">
            ...
            <v-card-actions>
              ...
              <v-btn color="success" :disabled="!valid" @click.prevent="update(service.id)">Editar</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-layout>
...
<script>
  export default
  {
    ...
    methods: {
      index(page, search) {
        axios.get("/admin/services?&search=" + search).then((response) => {
      ...
      store() {
        axios.post("/admin/services", this.service).then((response) => {
      ...
      edit(id) {
        axios.get("/admin/services/" + id + '/edit').then((response) => {
          if (response.status == 200) {
      ...
      update(id) {
        axios.patch("/admin/services/" + id, this.service).then((response) => {
          if (response.status == 200) {
            this.index(0, '');
            this.service = {};
            this.dialogedit = false;
            this.$swal.fire('Servicio actualizada con exito','success');
          }
        })
        .catch(error => {
      ...
      destroy() {
        axios.delete("/admin/services/" + this.id_delete).then((response) => {
          if (response.status == 200) {
            this.index(0, '');
            this.dialogdelete = false;
            this.$swal.fire('Servicio eliminada con exito','success');
          }
        }).catch(error => {
